...
while ($row = mysqli_fetch_array($q)) {
  ...
  if (isset($_SESSION["user_id"])) {
    ...
    if (mysqli_num_rows($q2) > 0) {
      echo "<div class='addfavbutton'><span id='lastchild'><button class='button'><a class='link' href='../favourite_products/add_to_favorites.php?product_id=$product_id'><img src='../images/heart-red-icon.png' style='width:20px;height:20px'></a></button></span></div> ";
    } else {
      echo "<div class='addfavbutton'><span id='lastchild'><button class='button'><a class='link' href='../favourite_products/add_to_favorites.php?product_id=$product_id'><img src='../images/heart-white-icon.png' style='width:20px;height:20px'></a></button></span></div> ";
    }

    if ($row['product_count'] > 0) {
      $q3 = mysqli_query($db, "SELECT * FROM `cart` WHERE user_id = $user_id AND product_id = $product_id");
      if (mysqli_num_rows($q3) > 0) {
        echo "<div class='backbutton'><span ><button class='button'><a class='link' href='add_to_cart.php?product_id=$product_id'><img src= '../images/cart-filled-icon.png' style='width:20px;height:20px'></a></button></span></div></center>";
      } else {
        echo "<div class='backbutton'><span ><button class='button'><a class='link' href='add_to_cart.php?product_id=$product_id'><img src= '../images/shopping-cart-icon.png' style='width:20px;height:20px'></a></button></span></div></center>";
      }
    }
     else {

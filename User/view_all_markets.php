...
while ($row=mysqli_fetch_array($q)) {
  ...
  echo "<div class='marketcard' >";
    ...
    $q2 = mysqli_query($db, "SELECT * FROM `liked_markets` WHERE user_id = $user_id AND market_id = $market_id");
    if (mysqli_num_rows($q2) > 0) {
      echo "<div class='addfavbutton'><span id='lastchild'><button class='button'><a href='add_to_liked_markets.php?market_id=$market_id'><img src='../images/heart-red-icon.png' style='width:20px;height:20px'></a></button></span></div> ";
    } else {
      echo "<div class='addfavbutton'><span id='lastchild'><button class='button'><a  href='add_to_liked_markets.php?market_id=$market_id'><img src='../images/heart-white-icon.png' style='width:20px;height:20px'></a></button></span></div> ";
    }
    echo "<br>";
    // echo"</a>";
//     if (isset($_SESSION["user_id"]))
